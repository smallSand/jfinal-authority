...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		if (user.getInt("status").equals(2)) throw new LockedAccountException(" USER Freeze");

		return new SimpleAuthenticationInfo(user.getStr("email"), user.getStr("pwd"), ByteSource.Util.bytes(user
				.getSalt()), getName());
	}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (user != null) {
			...
			info.addRoles(user.getRolesName(user.getStr("name")));
